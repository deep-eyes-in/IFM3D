...
{
	std::cerr << " PIC : " << filename << std::endl;
	...
	for (auto col = size_t{ 0 }; col < write_height; ++col) {
		for (auto row = size_t{ 0 }; row < write_width; ++row) {
			...
			std::cerr << write_pixels[pixel_index * 3 + 0] << std::endl;
			std::cerr << write_pixels[pixel_index * 3 + 1] << std::endl;
			std::cerr << write_pixels[pixel_index * 3 + 2] << std::endl;
...
//const std::string ifm3d::DEFAULT_IP = "192.168.0.69";
//const std::uint16_t ifm3d::DEFAULT_XMLRPC_PORT = 80;
//const std::string ifm3d::DEFAULT_PASSWORD = "";

const std::string DEFAULT_IP = "192.168.0.69";
const std::uint16_t DEFAULT_XMLRPC_PORT = 80;
...
int main(int argc, const char **argv)
{
	...
	auto cam = ifm3d::Camera::MakeShared(DEFAULT_IP, DEFAULT_XMLRPC_PORT, DEFAULT_PASSWORD) ;
/*
//	auto cam2 = ifm3d::Camera::MakeShared(DEFAULT_IP, DEFAULT_XMLRPC_PORT, DEFAULT_PASSWORD);
// get the JSON configuration data from the camera
	auto jsonConfig = cam2->ToJSON();
...
*/
	...
	ifm3d::FrameGrabber::Ptr fg =
		std::make_shared<ifm3d::FrameGrabber>(
			cam, ifm3d::IMG_RDIS);  // ifm3d::IMG_AMP | ifm3d::IMG_CART | 
	...
	ifm3d::SimpleImageBuffer::Img xyz_scaled;
	...
	if (distance.format == ifm3d::pixel_format::FORMAT_32F)
	{
		...
		std::cerr << "FORMAT_32F Format" << std::endl;
		scaleImageToRGB<float>(xyz, confidence, xyz_scaled, min_distance, max_distance);
	}
	...
	{	  //if data format is 16U then distances are in millimeters, Hence max distance is multiplied by 1000. 
		...
		std::cerr << "FORMAT_16U Format" << std::endl;
		scaleImageToRGB<float>(xyz, confidence, xyz_scaled, min_distance, max_distance * 1000 );
	}
	else
	{
...
*/
//  /*
	if (!writePPMFile(xyz_scaled, "xyzImage.ppm"))
	{
		std::cerr << "Not able to write the xyz data in ppm format" << std::endl;
		return -1;
	}
//  */
